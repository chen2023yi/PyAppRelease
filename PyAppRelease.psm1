...
function New-AutoInnoScript {
    ...
    $setupIconLine = if ($Icon -and (Test-Path $Icon)) { "SetupIconFile=`"{#IconPath}`"" } else { ""; }
    $shortcutIconName = '_pyapprelease_shortcut_{#AppVersion}.ico'
    $shortcutIconLine = if ($Icon -and (Test-Path $Icon)) {
        $filesEntries += "Source: `"{#IconPath}`"; DestDir: `"{app}`"; DestName: `"$shortcutIconName`"; Flags: ignoreversion"
        '; Shortcut icons are bound to an installed .ico so the desktop icon matches the user-selected icon.'
    } else {
        ''
    }
    $shortcutIconParam = if ($Icon -and (Test-Path $Icon)) { '; IconFilename: "{app}\' + $shortcutIconName + '"' } else { '' }
    $cleanupIconLines = if ($Icon -and (Test-Path $Icon)) {
        @(
            '[InstallDelete]',
            'Type: files; Name: "{app}\_pyapprelease_shortcut*.ico"',
            '',
            '[UninstallDelete]',
            'Type: files; Name: "{app}\_pyapprelease_shortcut*.ico"'
        ) -join "`r`n"
    ...
    $filesSection = $filesEntries -join "`r`n"
...
[Icons]
$shortcutIconLine
Name: "{group}\\{#AppName}"; Filename: "{app}\\{#AppName}.exe"$shortcutIconParam
Name: "{autodesktop}\\{#AppName}"; Filename: "{app}\\{#AppName}.exe"$shortcutIconParam; Tasks: desktopicon
...
'@
    ...
    # Resolve icon path: parameter overrides config; support relative & absolute paths
    $iconPath = $null
    $effectiveIcon = if ($Icon) { $Icon } else { $IconCfg }
    if ($effectiveIcon) {
        $resolvedIcon = if ([System.IO.Path]::IsPathRooted($effectiveIcon)) { $effectiveIcon } else { Join-Path $ProjectRoot $effectiveIcon }
        if (Test-Path $resolvedIcon) {
            $iconPath = $resolvedIcon
        } else {
            Write-ReleaseWarn "Icon file not found: $resolvedIcon"
        }
    }
    ...
    if (-not $DryRun) {
        # Upgrade PyInstaller and Pillow quietly. Pillow is required by
        # PyInstaller 6.x for icon conversion on Windows.
        $oldEap = $ErrorActionPreference
        $pipExit = 0
        if ($PSCmdlet -and $PSCmdlet.ShouldProcess($pythonExe, 'Install/upgrade pyinstaller + Pillow')) {
            try {
                $ErrorActionPreference = "Continue"
                & $pythonExe -m pip install --quiet --upgrade pyinstaller Pillow 2>&1 | ForEach-Object {
        ...
        if ($pipExit -and $pipExit -ne 0) {
            throw "pip install pyinstaller/Pillow failed with exit code $pipExit"
        }

        # Convert non-ICO images to ICO for PyInstaller / Inno Setup on Windows
        if ($iconPath -and ($iconPath -notmatch '\.ico$')) {
            $tempIco = Join-Path $outputDirPath "_pyapprelease_icon.ico"
            Write-ReleaseStep "Converting icon to ICO format"
            $convertPy = @'
import sys
...
try:
    ...
    dst = sys.argv[2]
    img = Image.open(src)
    # Ensure we have a clean RGBA image at a reasonable base size
    if img.mode not in ('RGBA', 'LA'):
        img = img.convert('RGBA')
    # Resize to 256x256 if smaller, preserve aspect ratio on larger images
    w, h = img.size
    if w < 256 or h < 256:
        ratio = min(256.0 / w, 256.0 / h)
        img = img.resize((int(w * ratio), int(h * ratio)), Image.LANCZOS)
    icons = [(256,256),(128,128),(64,64),(48,48),(32,32),(16,16)]
    img.save(dst, format='ICO', sizes=icons)
...
'@
            ...
            try {
                $ErrorActionPreference = "Continue"
                & $pythonExe -c $convertPy $iconPath $tempIco 2>&1 | ForEach-Object {
                    if ($_ -is [System.Management.Automation.ErrorRecord]) {
                        $msg = $_.ToString().Trim()
                        if ($msg) { Write-ReleaseWarn $msg }
                    } else {
            ...
            if ($convertExit -eq 0 -and (Test-Path $tempIco)) {
                $icoSize = (Get-Item $tempIco).Length
                Write-ReleaseOK "Icon converted: $(Split-Path $tempIco -Leaf) ($icoSize bytes)"
                $iconPath = $tempIco
            } else {
                Write-ReleaseWarn "Failed to convert icon to ICO. Install Pillow: pip install Pillow"
                $iconPath = $null
            }
        }
    ...
    if ($iconPath) { $isccArgs += "/DIconPath=$iconPath" }
...
#>
    ...
    $iconLine = if ($Icon) {
        ...
        "    # Icon         = `"assets\$AppName.ico`"       # optional EXE / installer icon"
    }
