...
.DESCRIPTION
    ...
    builds the EXE with PyInstaller, optionally creates an Inno Setup installer,
...
#>
    ...
    if (-not (Test-Path $cfgPath)) {
        throw "Release config not found: $cfgPath`nCreate one with: New-PyAppReleaseConfig -AppName YourApp"
    }
    ...
    $installerPath = ""
    if ($InnoScript -eq '') {
        Write-ReleaseSkip "No InnoScript configured"
    } else {
